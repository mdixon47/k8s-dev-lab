...
| `scripts/control-plane.sh` | `kubeadm init`, writes kubeconfig to `/vagrant/kubeconfig`, installs Flannel pinned to the interface that owns `NODE_IP` (`eth1`; the bento box boots with `net.ifnames=0`), writes `/vagrant/join.sh`. |
...
make up        # open the VirtualBox app (make vbox), then vagrant up (10–15 min first run; VBOX_APP=0 skips the app)
