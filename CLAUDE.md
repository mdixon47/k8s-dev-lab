...
| `scripts/worker.sh` | Waits for `join.sh`, then for the API server it names to answer `/healthz`, then `kubeadm join` with up to 3 attempts (`kubeadm reset -f` between them). Skips if `kubelet.conf` exists. |
...
| `Makefile` | Workflow targets (below). `test` derives the worker and edge IPs from the Vagrantfile (`NET_PREFIX`, first `worker`/`web` node); `deploy` waits `ROLLOUT_TIMEOUT` (180s) per rollout; `status` warns when fewer nodes are present than the Vagrantfile's control-plane + worker entries (`CLUSTER_NODES`). |
| `scripts/check.sh` | `make check`: ShellCheck on every script, yamllint (`.yamllint`: relaxed, line length and colon alignment off) on the manifests and `.github/`, actionlint on `.github/workflows/`, kubeconform on `k8s/`, `policy/examples/`, `policy/tests/fixtures/`, `security/policies/` (unknown CRD kinds skipped), and a relative-link check over every `*.md`. Missing shellcheck/yamllint → SKIP; actionlint and kubeconform fall back to their Docker images. |
...
| `.github/workflows/check.yml` | CI: `make check` + `make policy-test` on push and PR. Read-only token, superseded runs on the same ref are cancelled. No VM steps. |
| `scripts/gateway.sh` | `make gateway`: Helm installs of MetalLB (`METALLB_VERSION`), Envoy Gateway (`ENVOY_GATEWAY_VERSION`) and Istio base + istiod (`ISTIO_VERSION`, istiod memory request 256Mi), then `gateway/`. Gateway API CRDs (standard channel) come from Envoy Gateway's `gateway-crds-helm` chart, applied separately; on API servers < 1.31 the script strips TLSRoute's `isIP()` CEL rule, which 1.30 rejects. `uninstall` reverses it (CRDs stay). Needs `helm`. |
| `gateway/` | Gateway API objects: `00-namespaces.yaml` (labelled `owner: lab`, created before the charts so a namespace-label deny constraint does not block them), `10-metallb-pool.yaml` (L2, `192.168.56.100-110`), `20-envoy-gateway.yaml` (GatewayClass `eg` + Gateway on `.100`), `30-istio-gateway.yaml` (Gateway class `istio` on `.101`; istiod deploys `istio-istio` into `gateways`), `40-routes.yaml` (HTTPRoutes `api`, `web` in `devapp`, parentRefs to both). `examples/`: `envoy-ratelimit.yaml` (BackendTrafficPolicy), `istio-authz.yaml` (AuthorizationPolicy denying /docs), `edge-gateway.conf` (nginx for the web VM pointing at the gateways; `proxy_http_version 1.1` is required, Envoy answers HTTP/1.0 with 426). Not applied by `make gateway`. |
| `docs/gateway-course.md` | Ten-lesson training: Gateway API roles, MetalLB, Envoy Gateway, HTTPRoute attachment, Istio ingress, TLS on both, implementation policies, Envoy admin APIs and failure drills, edge capstone. Verified on the lab 2026-09-21 (Kubernetes 1.30.14). |
| `docs/issues.md` | Known gaps, unverified claims and future work, each with status. Update it when one is resolved. |
| `scripts/gatekeeper.sh` | `make policy`: applies the pinned OPA Gatekeeper release manifest (`GATEKEEPER_VERSION`, Makefile) with the controller's `replicas: 3` rewritten to 1 before the apply (3×512Mi does not fit a 2 GB worker; patching the manifest rather than scaling afterwards means a re-run resets a stray scale-up), guarded so an upstream format change fails loudly, applies `policy/templates/` then waits for their CRDs, then `policy/constraints/`. `uninstall` argument removes all of it. |
...
make gateway   # MetalLB + Envoy Gateway + Istio + gateway/ (docs/gateway-course.md); gateway-uninstall removes it
...
- `make deploy` fails with `error: timed out waiting for the condition`: a rollout did not finish within `ROLLOUT_TIMEOUT`. Postgres → PVC Pending (`make storage`); api/web → image missing (`make image`) or `/readyz` failing. Pods `Pending` with `Insufficient memory` in `kubectl -n devapp get events`: a worker is missing from `kubectl get nodes` (rejoin it, see below) or Gatekeeper is running 3 controller replicas again (`kubectl -n gatekeeper-system scale deploy gatekeeper-controller-manager --replicas=1`; `kubectl describe node w2 | grep -A8 'Allocated resources'` shows the squeeze). Fix and re-run `make deploy`.
- `make gateway` fails installing CRDs with `undeclared reference to 'isIP'` (or `format`): the API server is older than 1.31 and the Gateway API CRDs use newer CEL functions. The script already strips the one standard-channel rule affected (TLSRoute); the experimental channel needs 1.32+. Rebuild on 1.36 (`K8S_VERSION`) for the full set.
- `make gateway` fails with `chart "base" matching X not found in istio index`: Istio's Helm charts lag its GitHub release by a few days; `helm search repo istio/base --versions | head` and set `ISTIO_VERSION` to a published one.
- A gateway address (`192.168.56.100`/`.101`) does not answer while `kubectl -n gateways get gateway` shows `Programmed True`: check `kubectl -n metallb-system get servicel2status` names a node and that the proxy Deployment has replicas (`kubectl -n envoy-gateway-system get deploy`, `kubectl -n gateways get deploy`). `426 Upgrade Required` from a gateway means the client sent HTTP/1.0 (nginx without `proxy_http_version 1.1`).
