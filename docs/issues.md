...
| 2.3 | open | **`make check` in CI.** There is no workflow file. `check` and `policy-test` both run without VMs and finish in seconds (Docker fallbacks pull `ghcr.io/yannh/kubeconform` and `openpolicyagent/gator`), so a GitHub Actions job is cheap. |
