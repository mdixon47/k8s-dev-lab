...
| 1.8 | open | **learn.md exercise 11 and course lesson 3: the `/swap.img` fstab line.** `common.sh` comments out whichever line has `swap` in it, so provisioning is fine either way, but the exercise tells learners to uncomment `/swap.img` by name. Confirm that is the bento/ubuntu-26.04 swap file name (`grep swap /etc/fstab` on any node). |
...
| 2.3 | resolved 2026-09-20 | **`make check` in CI.** `.github/workflows/check.yml` runs `make check` and `make policy-test` on every push and pull request (ubuntu-latest; shellcheck and yamllint from apt, kubeconform and gator from their Docker images). |
