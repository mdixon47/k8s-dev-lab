...
| 2.7 | open | **`make deploy` timed out on a real lab (2026-09-20)** because w1 had been reset (exercise 16) and never rejoined, and Gatekeeper's controller was back at 3 replicas (1536 Mi requested), which filled w2 to 96% so the api pods stayed `Pending: Insufficient memory`. Fixed by hand (scale to 1, `vagrant provision cp1 --provision-with control-plane` for a fresh token, `vagrant provision w1 --provision-with worker`). Two follow-ups: (a) `make status` could print `kubectl get nodes` first and flag a node count below `NODES`; (b) how the controller returned to 3 replicas is unknown; `scripts/gatekeeper.sh` scales after every apply, so it was not `make policy`. Watch for it; if it recurs, patch the Deployment's replicas in the manifest before applying instead of scaling afterwards. |
